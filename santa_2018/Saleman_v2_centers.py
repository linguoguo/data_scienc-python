...
i=2
nearest_point(df[df['mclust']==i],centers.iloc[i+1])
...
t=nearest_point(df[df['mclust']==0],centers.iloc[1][['X','Y']])
...
for i in range(1,len(centers)+1):
    a=route2.iloc[i].mclust
    b=route2.iloc[i-1].mclust
    print (b)
    if np.isnan(b):
        coord=nearest_point(df[df['mclust']==a],north_pole[['X','Y']])
        route2.loc[i,'start_point']=coord
    else:        
    ...
    if np.isnan(b):
        ...
        coord=nearest_point(df[df['mclust']==a],centers.iloc[b][['X','Y']])
    route2.loc[i,'end_point']=coord    
    
    print(a,'b',b,'coord',coord)