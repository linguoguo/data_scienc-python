...
b=len(cities)
num=random.sample(range(1,b ), 150)
num=[0]+num
df=cities.iloc[num]
...
def tol(df,d_tol):
    ...
    for i in range(0,len(df)-1):
        dis=distance(df.iloc[i][['X','Y']],df.iloc[i+1][['X','Y']])
        p=d_tol[d_tol['CityId']==df.iloc[i].CityId].index
        if (p+1)%10==0 and df['prime'][i]==False:
            d+=1.1*dis
        else:
            d+=dis
    return d
...
def tol_entr(df,d_tol,ent,sorti):
...
for i in range(1,n_cluster+1):
    ...
    solve_tsp(df_o,0)
    df_tol=df_tol.append(df_o).reset_index(drop=True)
...
#-------------- mergeinto v5 ----------------------------
def mergeinto(df,d_tol,pt,entr,sorti):
    l=len(df)
   ...
   # print (dc)    
    ...
    dist=[tol_entr(d0,d_tol,entr,sorti),tol_entr(d1,d_tol,entr,sorti),tol_entr(d2,d_tol,entr,sorti),tol_entr(d3,d_tol,entr,sorti),tol_entr(d4,d_tol,entr,sorti)]  
...
for i in range(len(d2)):
    print(i)
    d1=mergeinto(d1,df_tol,d2.iloc[i],north_pole,north_pole)
    plt_lines(d1)
i=7    
...
#----------------------------best_s_e v2-----------------------------------------
def best_s_e(df,d_tol,cent,entr,sorti,ls,le):
